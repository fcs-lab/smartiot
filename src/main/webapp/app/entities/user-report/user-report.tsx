  ...
  return (
    <div>
      <h2 id="user-report-heading" data-cy="UserReportHeading">
        <Translate contentKey="smartiotApp.userReport.home.title">User Reports</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="smartiotApp.userReport.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/user-report/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="smartiotApp.userReport.home.createLabel">Create new User Report</Translate>
          </Link>
        </div>
      </h2>
        ...
        {userReportList && userReportList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="smartiotApp.userReport.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('reportContent')}>
                  <Translate contentKey="smartiotApp.userReport.reportContent">Report Content</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('reportContent')} />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {userReportList.map((userReport, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{userReport.reportType}</td>
                  <td>
                    {userReport.generatedAt ? <TextFormat type="date" value={userReport.generatedAt} format={APP_DATE_FORMAT} /> : null}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="smartiotApp.userReport.home.notFound">No User Reports found</Translate>
            </div>
          )
        )}
      </div>
