  ...
  return (
    <div>
      <h2 id="alert-heading" data-cy="AlertHeading">
        <Translate contentKey="smartiotApp.alert.home.title">Alerts</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="smartiotApp.alert.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/alert/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="smartiotApp.alert.home.createLabel">Create new Alert</Translate>
          </Link>
        </div>
      </h2>
        ...
        {alertList && alertList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="smartiotApp.alert.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="smartiotApp.alert.consumer">Consumer</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {alertList.map((alert, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{alert.createdDate ? <TextFormat type="date" value={alert.createdDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{alert.consumer ? <Link to={`/consumer/${alert.consumer.id}`}>{alert.consumer.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="smartiotApp.alert.home.notFound">No Alerts found</Translate>
            </div>
          )
        )}
      </div>
